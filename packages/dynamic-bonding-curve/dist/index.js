...
var VIRTUAL_SWAP_AUTHORITY = new PublicKey(
...
function getTokenProgram(tokenType) {
  return tokenType === 0 /* SPLToken */ ? TOKEN_PROGRAM_ID : TOKEN_2022_PROGRAM_ID;
}
async function getTokenType(connection, tokenMint) {
  ...
  return accountInfo.owner.equals(TOKEN_PROGRAM_ID) ? 0 /* SPLToken */ : 1 /* Token2022 */;
}
...
function isCurveComplete(virtualPool, config) {
  const totalQuoteReserve = virtualPool.poolState.quoteReserve.add(
    virtualPool.poolState.virtualQuoteReserve ?? new BN11(0)
  );
...
function isSaleComplete(virtualPool, config, currentTimestamp) {
  ...
  const deadlineTimestamp = virtualPool.poolState.deadlineTimestamp ?? new BN11(0);
...
function validatePoolFees(poolFees, collectFeeMode, activationType) {
  ...
  if (!validateDynamicFee(poolFees.dynamicFee ?? void 0)) {
    return false;
  }
  return true;
}
...
function validateTransferHookProgram(transferHookProgram) {
  ...
  if (!(transferHookProgram instanceof PublicKey6)) {
    return false;
  }
  if (transferHookProgram.equals(PublicKey6.default)) {
    return false;
  }
  if (transferHookProgram.equals(DYNAMIC_BONDING_CURVE_PROGRAM_ID)) {
    return false;
  }
  if (transferHookProgram.equals(TOKEN_PROGRAM_ID2)) {
    return false;
  }
  if (transferHookProgram.equals(TOKEN_2022_PROGRAM_ID2)) {
    return false;
  }
  return true;
}
...
function validateQuoteMintBasic(quoteMint) {
  if (!quoteMint || quoteMint.equals(PublicKey6.default)) {
    return false;
  }
...
function validateLiquidityVestingInfo(vestingInfo) {
  ...
  const totalVestedLiquidity = U128_MAX.mul(
    new BN13(vestingInfo.vestingPercentage)
  ).div(new BN13(100));
  const totalVestingLiquidityAfterCliff = totalVestedLiquidity.mul(new BN13(totalBpsAfterCliff)).div(new BN13(MAX_BASIS_POINT));
  const liquidityPerPeriod = vestingInfo.numberOfPeriods > 0 ? totalVestingLiquidityAfterCliff.div(
    new BN13(vestingInfo.numberOfPeriods)
  ) : new BN13(0);
  ...
  if (liquidityPerPeriod.gt(new BN13(0))) {
  ...
  const totalPeriodicLiquidity = liquidityPerPeriod.mul(
    new BN13(effectiveNumberOfPeriods)
  );
  ...
  return vestingDuration <= MAX_LOCK_DURATION_IN_SECONDS && cliffUnlockLiquidity.add(totalPeriodicLiquidity).gt(new BN13(0));
}
...
function validateConfigParameters(configParam, options = false) {
  ...
  if (swapBaseAmountForCurve.lte(new BN13(0)) || migrationBaseAmountForCurve.lte(new BN13(0))) {
    throw new Error(
...
function validateDeadlineTimestamp(deadlineTimestamp) {
  if (!deadlineTimestamp.isZero() && deadlineTimestamp.lte(new BN13(Math.floor(Date.now() / 1e3)))) {
...
function validateMigratedPoolBaseFeeMode(migratedPoolBaseFeeMode, migratedPoolMarketCapFeeSchedulerParams, migrationOption, poolFeeBps = MIN_MIGRATED_POOL_FEE_BPS) {
  ...
  if (migratedPoolBaseFeeMode === 3 /* FeeMarketCapSchedulerLinear */ || migratedPoolBaseFeeMode === 4 /* FeeMarketCapSchedulerExponential */) {
    ...
    if (minFeeNumerator.lt(new BN13(DAMM_V2_MIN_FEE_NUMERATOR)) || cliffFeeNumerator.gt(new BN13(DAMM_V2_MAX_FEE_NUMERATOR))) {
...
function buildCurve(params) {
  ...
  const instructionParams = {
    ...
    migrationQuoteAmountCap: migration.migrationQuoteAmountCap ?? new BN14(0),
    sqrtStartPrice,
...
var DynamicBondingCurveProgram = class {
  ...
  prepareSwapParams(swapBaseForQuote, virtualPoolState, poolConfigState) {
    if (swapBaseForQuote) {
      return {
        inputMint: new PublicKey8(virtualPoolState.baseMint),
        outputMint: new PublicKey8(poolConfigState.quoteMint),
    ...
    return {
      inputMint: new PublicKey8(poolConfigState.quoteMint),
      outputMint: new PublicKey8(virtualPoolState.baseMint),
  ...
  async buildCreatePoolTx(createPoolParam, tokenType, quoteMint) {
    ...
    const deadlineTimestamp = createPoolParam.deadlineTimestamp ?? new BN15(0);
  ...
  async getRemainingAccountsForTransferHook(mint, accountTypes = [AccountsType.TransferHookBase]) {
    ...
    if (mintInfo.owner.equals(TOKEN_PROGRAM_ID4)) {
      return emptyAccounts;
    }
    const mintState = unpackMint(mint, mintInfo, TOKEN_2022_PROGRAM_ID3);
    const transferHook = getTransferHook(mintState);
    if (!transferHook || transferHook.programId.equals(PublicKey8.default)) {
      return emptyAccounts;
    }
...
var StateService = class extends DynamicBondingCurveProgram {
  ...
  async getPoolConfig(configAddress) {
    const address = configAddress instanceof PublicKey9 ? configAddress : new PublicKey9(configAddress);
    try {
  ...
  async getPool(poolAddress) {
    const address = poolAddress instanceof PublicKey9 ? poolAddress : new PublicKey9(poolAddress);
    try {
  ...
  async getPoolFeeBreakdown(poolAddress) {
    ...
    let creatorTotalTradingBaseFee = new BN16(0);
    let creatorTotalTradingQuoteFee = new BN16(0);
    ...
    if (creatorTradingFeePercentage > 0) {
      creatorTotalTradingBaseFee = totalTradingBaseFee.mul(new BN16(creatorTradingFeePercentage)).div(new BN16(100));
      creatorTotalTradingQuoteFee = totalTradingQuoteFee.mul(new BN16(creatorTradingFeePercentage)).div(new BN16(100));
...
var MigrationService = class extends DynamicBondingCurveProgram {
  ...
  async createLocker(params) {
    ...
    const preInstructions = [];
    const createOwnerEscrowVaultTokenXIx = createAssociatedTokenAccountIdempotentInstruction3(
      payer,
      escrowToken,
      escrow,
      virtualPool.poolState.baseMint,
  ...
  async lockDammV1LpToken(params) {
    ...
    const createEscrowVaultIx = createAssociatedTokenAccountIdempotentInstruction3(
      payer,
      escrowVault,
      lockEscrowKey,
      lpMint,
  ...
  async claimDammV1LpToken(params) {
    ...
    const preInstructions = [];
    const createDestinationTokenIx = createAssociatedTokenAccountIdempotentInstruction3(
      payer,
      destinationToken,
...
var PartnerService = class extends DynamicBondingCurveProgram {
  ...
  async createConfig(params) {
    ...
    return this.buildCreateConfigTx(
      configParam,
      new PublicKey11(config),
      new PublicKey11(feeClaimer),
      new PublicKey11(leftoverReceiver),
      new PublicKey11(quoteMint),
      new PublicKey11(payer)
    );
  }
  /**
  ...
  async createConfigWithTransferHook(params) {
    ...
    return this.buildCreateConfigWithTransferHookTx(
      ...
      new PublicKey11(leftoverReceiver),
      new PublicKey11(quoteMint),
      new PublicKey11(transferHookProgram),
      new PublicKey11(payer)
    );
  }
  /**
    ...
    const createPoolTx = await this.buildCreatePoolWithTransferHookTx(
      {
        ...
        config: configKey,
        payer: payerAddress,
        transferHookProgram: new PublicKey11(transferHookProgram)
      },
      quoteMintToken,
      tokenQuoteProgram
    );
  ...
  async createConfigAndPoolWithFirstBuy(params) {
    ...
    if (firstBuyParam && firstBuyParam.buyAmount.gt(new BN17(0))) {
  ...
  async createConfigAndPoolWithFirstBuyWithTransferHook(params) {
    ...
    const quoteMintToken = new PublicKey11(quoteMint);
    const payerAddress = new PublicKey11(payer);
    const transferHookProgramKey = new PublicKey11(transferHookProgram);
    const createConfigTx = await this.buildCreateConfigWithTransferHookTx(
      configParam,
      configKey,
      new PublicKey11(feeClaimer),
...
var PoolService = class extends DynamicBondingCurveProgram {
  ...
  async swap(params) {
    ...
    if (poolConfigState.poolFees.baseFee.baseFeeMode === 2 /* RateLimiter */) {
      ...
      rateLimiterApplied = isRateLimiterApplied(
        ...
        new BN18(poolConfigState.poolFees.baseFee.firstFactor)
      );
    }
    const { inputMint, outputMint, inputTokenProgram, outputTokenProgram } = this.prepareSwapParams(
      swapBaseForQuote,
    ...
    if (inputMint.equals(NATIVE_MINT5)) {
      preInstructions.push(
        ...wrapSOLInstruction(
          owner,
          inputTokenAccount,
    ...
    if (rateLimiterApplied || poolConfigState.enableFirstSwapWithMinFee) {
      remainingAccounts.push({
        pubkey: SYSVAR_INSTRUCTIONS_PUBKEY2,
        isSigner: false,
        isWritable: false
      });
    }
  ...
  normalizeQuoteConfig(config) {
    ...
    const migrationSqrtPrice = config.migrationSqrtPrice ?? getMigrationThresholdPrice(
  ...
  buildSimulatedVirtualPool(sqrtStartPrice) {
    return {
      poolState: {
        sqrtPrice: new BN18(sqrtStartPrice),
        baseReserve: new BN18(0),
        quoteReserve: new BN18(0),
        virtualQuoteReserve: new BN18(0),
        deadlineTimestamp: new BN18(0),
        activationPoint: new BN18(0),
        volatilityTracker: {
          lastUpdateTimestamp: new BN18(0),
          sqrtPriceReference: new BN18(0),
          volatilityAccumulator: new BN18(0),
          volatilityReference: new BN18(0),
          padding: []
        }
      }
    };
...
var CreatorService = class extends DynamicBondingCurveProgram {
  ...
  async createPoolWithFirstBuy(params) {
    ...
    if (firstBuyParam && firstBuyParam.buyAmount.gt(new BN19(0))) {
      const swapBuyTx = await this.buildSwapBuyTx(
        firstBuyParam,
  ...
  async createPoolWithPartnerAndCreatorFirstBuy(params) {
    ...
    if (partnerFirstBuyParam && partnerFirstBuyParam.buyAmount.gt(new BN19(0))) {
    ...
    if (creatorFirstBuyParam && creatorFirstBuyParam.buyAmount.gt(new BN19(0))) {
...
export {
  ...
  validatePoolFees,
  validateQuoteMintBasic,
  validateSwapAmount,
  validateTokenAuthorityOptions,
